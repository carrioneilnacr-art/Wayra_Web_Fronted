...
export const ModalDetalleTicket = ({ ticket, onClose }) => {
  // Manejo de la tecla Escape para cerrar (Reliability & UX)
  useEffect(() => {
  ...
  if (!ticket) return null; // Defensive programming para evitar crashes
...
// 📅 MODAL 2: SELECTOR DE SESIONES (Nativamente Accesible)
  ...
  return (
    ...
    >
      <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-6 md:p-8 text-center border border-slate-100 flex flex-col max-h-[85vh]">
        ...
        <ul className="flex-1 overflow-y-auto space-y-2.5 pr-1 scrollbar-thin scrollbar-thumb-slate-200" role="list">
          {lista.map(r => (
            <li key={r.id_reserva}>
              <button 
                type="button"
