...
const ModalReserva = ({ mesa, onClose, onSave, todasLasReservas, fechaSeleccionada, reservaEdit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!reservaEdit && form.fecha_reserva === hoyStr) {
      try {
        // ✅ ARQUITECTURA SENIOR: Consumo a través del servicio desacoplado
  ...
  return (
    // ✅ Overlay purificado a Tailwind con perfecta concordancia de desenfoque y opacidad
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[2000] flex items-center justify-center p-4 uppercase italic font-black animate-in fade-in duration-200">
      ...
      {/* ✅ modalCard convertido a contenedor elástico */}
              ...
              {turnos.map(t => {
                ...
                return (
                  <button 
                    ...
                    className={`p-3 rounded-2xl text-[10px] border flex flex-col items-center justify-center transition-all duration-300 gap-0.5 uppercase font-black shrink-0
                      ${bloqueado 
                        ? 'bg-slate-100 border-slate-100 text-slate-300 opacity-50 cursor-not-allowed' 
                        : seleccionado 
                          ? 'bg-blue-600 border-blue-600 text-white scale-105 shadow-md shadow-blue-600/20' 
                  ...
                  >
                    ...
                    {ocupado ? <span className="text-[6px] font-black text-rose-500 not-italic font-sans">OCUPADO</span> : 
                     yaPaso ? <span className="text-[6px] font-black text-slate-400 not-italic font-sans">PASADO</span> : 
                     <span className={`text-[6px] font-black not-italic font-sans ${seleccionado ? 'text-blue-200' : 'text-emerald-500'}`}>DISPONIBLE</span>}
                  </button>
                );
              })}
            </div>
