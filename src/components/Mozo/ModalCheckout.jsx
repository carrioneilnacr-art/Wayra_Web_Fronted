...
export default function ModalCheckout({ pedido, reservas, onClose, onSuccess }) {
  ...
  // Botón deshabilitado hasta que se cumplan las reglas
  const isFormValid = () => {
  ...
  return (
    <div className="fixed inset-0 bg-[#1a1a1a]/60 backdrop-blur-sm z-[3000] flex items-center justify-center p-4 animate-in fade-in duration-300 print:bg-white print:backdrop-blur-none">
        ...
        <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 print:hidden">
          ...
          <div className="p-6 space-y-5">
            <div className="flex bg-slate-100 p-1 rounded-xl">
              <button onClick={() => setTipoDoc('BOLETA')} className={`flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all ${tipoDoc === 'BOLETA' ? 'bg-white shadow-sm text-[#1a1a1a]' : 'text-slate-400'}`}>Boleta</button>
            ...
            <button 
              ...
              className={`w-full font-black text-[11px] tracking-[0.2em] py-4 rounded-xl uppercase transition-all mt-4 ${isFormValid() ? 'bg-[#c5a059] text-white hover:bg-[#b07d62] shadow-lg shadow-[#c5a059]/20' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
            >
              {isFormValid() ? 'Continuar al Pago' : 'Complete los datos'}
            </button>
          </div>
        </div>
      )}
      ...
      {/* =========================================
          PASO 2: PASARELA TIPO NIUBIZ (MINIMALISTA)
          ========================================= */}
      {step === 2 && (
        <div className="bg-white w-full max-w-[360px] rounded-[2rem] shadow-2xl overflow-hidden animate-in slide-in-from-right-8 relative print:hidden border border-slate-100">
          ...
          <div className="p-6">
            ...
            <div className="flex gap-2">
              <button onClick={() => setStep(1)} className="py-4 px-4 bg-slate-100 text-slate-500 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-colors">Volver</button>
              <button 
                onClick={procesarPagoFinal} 
                disabled={procesando}
                className={`flex-1 text-white font-black text-xs tracking-widest py-4 rounded-xl uppercase shadow-lg transition-all ${procesando ? 'bg-slate-400' : metodoPago === 'YAPE' ? 'bg-[#742384]' : metodoPago === 'PLIN' ? 'bg-teal-500' : metodoPago === 'EFECTIVO' ? 'bg-emerald-500' : 'bg-[#00a1e1]'}`}
              >
      ...
      {/* =========================================
          PASO 3: TU BOLETA KINTSUGI (MEJORADA)
          ========================================= */}
      {step === 3 && (
        <div className="max-w-md w-full bg-[#fcfaf7] rounded-[2.5rem] shadow-2xl relative overflow-hidden border border-slate-100 print:shadow-none print:border-none animate-in slide-in-from-bottom-8 duration-500">
          ...
          <div className="flex border-t border-slate-100 print:hidden shrink-0">
            <button onClick={() => window.print()} className="flex-1 py-4.5 bg-slate-50 text-slate-700 text-[9px] tracking-[0.3em] uppercase font-black hover:bg-[#b07d62] hover:text-white transition-all duration-300 border-r border-slate-100 flex items-center justify-center gap-1">
              🖨️ Imprimir
            </button>
            <button onClick={onClose} className="flex-1 py-4.5 bg-slate-50 text-rose-600 text-[9px] tracking-[0.3em] uppercase font-black hover:bg-rose-600 hover:text-white transition-all duration-300 flex items-center justify-center gap-1">
              ✕ Liberar Mesa
            </button>
          </div>
        </div>
