...
export const PanelDerechoReservas = ({
  ...
  const handleLlegadaCliente = async (reserva) => {
    try {
      ...
      console.error("Error al procesar el ingreso del cliente:", e);
      alert("Error al procesar el ingreso del cliente.");
    }
  };

  const handleAnular = async (id) => {
    if (!globalThis.confirm("¿Seguro de cancelar esta reserva?")) return;
    try {
      await wayraApi.put(`/reservas/${id}/cancelar`);
      onCheckIn();
    } catch (e) {
      console.error("Error al anular la reserva:", e);
    ...
    <div className="w-full h-full flex flex-col bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-slate-50/50">
        {reservas.length > 0 ? reservas.map((r) => {
          ...
          // Evitamos ternario anidado para la clase de fondo del badge de estado
          let badgeBgClass = 'bg-slate-400';
          if (semaforo.color === 'emerald') {
            badgeBgClass = 'bg-emerald-500';
          } else if (semaforo.color === 'rose') {
            badgeBgClass = 'bg-rose-500 animate-pulse';
          } else if (semaforo.color === 'amber') {
            badgeBgClass = 'bg-amber-500 animate-pulse';
          } else if (semaforo.color === 'blue') {
          ...
          return (
            <div
              key={r.id_reserva}
              className={`p-4 rounded-3xl border-2 transition-all bg-white flex flex-col gap-4 shadow-sm ${semaforo.color === 'rose' ? 'border-rose-100 hover:border-rose-300' : 'border-slate-100 hover:border-slate-200'
                }`}
            >
              <div className="flex justify-between items-start w-full">
                <div className="min-w-0 flex-1 pr-2">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className={`text-[7px] font-black px-2 py-0.5 rounded-md text-white tracking-widest leading-none ${badgeBgClass}`}>
                      {semaforo.label}
                    </span>
                    {r.id_pedido && (
                      <button
              ...
              {fechaSeleccionada === hoyStr && r.estado_reserva === 'pendiente' && (
                <button
                  ...
                  className={`w-full py-4 rounded-2xl text-[10px] font-black text-white shadow-lg transition-all ${semaforo.color === 'rose' ? 'bg-rose-600 hover:bg-rose-700 animate-pulse' : 'bg-slate-900 hover:bg-blue-600'
                    }`}
                >
                  LLEGÓ ✔
                </button>
...
PanelDerechoReservas.propTypes = {
  reservas: PropTypes.arrayOf(
    PropTypes.shape({
      id_reserva: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
      ...
      nombre_mozo: PropTypes.string,
      id_pedido: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    })
  ).isRequired,
  ...
  hoyStr: PropTypes.string.isRequired,
