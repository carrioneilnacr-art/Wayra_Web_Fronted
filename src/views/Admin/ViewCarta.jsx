...
export const ViewCarta = () => {
  ...
  // ⚙️ Estado de control de edición integral
  ...
  // Diccionario de Iconos SVG Minimalistas
  const obtenerIconoCategoria = (cat, esDisponible) => {
    const colorClass = esDisponible ? 'text-[#b07d62]' : 'text-slate-400';
    switch (cat.toUpperCase()) {
      case 'ENTRADAS':
        return <svg className={`w-5 h-5 ${colorClass}`} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 3v18M3 12h18M6 6l12 12M6 18L12 12" /></svg>;
      case 'MAKIS':
        return <svg className={`w-5 h-5 ${colorClass}`} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="9" /><circle cx="12" cy="12" r="4" strokeDasharray="2 2" /></svg>;
      case 'FONDOS':
        return <svg className={`w-5 h-5 ${colorClass}`} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12M6 12h12" /></svg>;
      case 'BEBIDAS':
        return <svg className={`w-5 h-5 ${colorClass}`} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M6 7.5h12M6 16.5h12" /></svg>;
      case 'POSTRES':
        return <svg className={`w-5 h-5 ${colorClass}`} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>;
      default:
  ...
  // ✅ ARQUITECTURA SENIOR: Consumo del servicio aislado
  ...
  const handleUpdateProductoCompleto = async (e) => {
    e.preventDefault();
    try {
      await productoService.update(modalEditar.idProducto, {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      await productoService.create(nuevoProd); 
    ...
    <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-6 text-[#2C3E50]">
      ...
      <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 w-full">
          ...
          {categorias.map(cat => (
            <button 
              ...
              className={`px-4 py-2 rounded-full text-[10px] tracking-[0.2em] font-bold uppercase transition-all border
                ${catSel === cat 
                  ? 'bg-[#b07d62]/15 text-[#b07d62] border-[#b07d62]/30 shadow-sm' 
        ...
        <button 
          ...
          className="bg-[#0a0913] text-white py-3 px-5 rounded-full font-bold text-[10px] tracking-[0.2em] uppercase shadow-sm hover:bg-[#2d2a45] transition-all flex items-center gap-2 w-full xl:w-auto justify-center md:w-auto"
        >
      ...
      <div className="relative">
        <input 
          ...
          className="w-full bg-white border border-slate-100 p-4 pl-5 rounded-2xl text-[10px] text-[#2C3E50] placeholder-slate-400 outline-none focus:border-[#b07d62]/40 font-bold uppercase tracking-widest transition-all shadow-sm"
          onChange={(e) => setBusqueda(e.target.value)}
        />
      </div>

      {/* 🍱 LISTADO DE PLATOS - 🌟 GRID RESPONSIVO MULTICOLUMNA (1 móvil, 2 tablet mozo, 3 desktop) */}
        ...
        {productos
          ...
          .map(p => {
            ...
            return (
              <div 
                key={p.id_producto} 
                className={`bg-white p-5 rounded-2xl border transition-all flex items-center justify-between shadow-[0_2px_8px_rgba(0,0,0,0.01)] h-24
                  ${esDisponible ? 'border-slate-100' : 'border-rose-200/60 bg-rose-50/20 opacity-85'}`}
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button 
                    onClick={() => handleUpdateEstado(p.id_producto, esDisponible ? 0 : 1)}
                    className={`px-2.5 py-1.5 rounded-full text-[8px] font-black tracking-widest border transition-all text-center w-20
                      ${esDisponible 
                        ? 'border-emerald-200 text-emerald-600 bg-emerald-50/50 hover:bg-emerald-100/50' 
                        : 'border-rose-200 text-rose-500 bg-rose-50/50 hover:bg-rose-100/50'}`}
                  >
      ...
      {/* ⚙️ MODAL 2: EDICIÓN INTEGRAL DE DATOS */}
      {modalEditar.abierto && (
        <div className="fixed inset-0 bg-[#0a0913]/40 flex items-center justify-center z-50 p-4 backdrop-blur-md transition-all">
          <form onSubmit={handleUpdateProductoCompleto} className="bg-white p-7 rounded-2xl border border-slate-100 w-full max-w-sm shadow-xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3 mt-6">
              <button type="button" onClick={() => setModalEditar({ abierto: false, idProducto: null, nombre: "", precio: "", categoria: "Entradas", tiempo_estimado: "" })} className="flex-1 p-3 text-[#7F8C8D] font-bold hover:text-[#2C3E50] transition-colors text-[10px] tracking-widest">CANCELAR</button>
