...
export const ViewUsuarios = () => {
  ...
  const cargarUsuarios = async () => {
    try {
      ...
      // Forzamos la validación de que sea un arreglo para que el .map() nunca falle
      if (res.data && Array.isArray(res.data)) {
        setUsuarios(res.data);
      } else {
        console.warn("La API no devolvió un array directo:", res.data);
      }
    } catch (e) { 
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-6 text-[#2C3E50]">
      ...
      {/* 💳 GRID DE ID CARDS ESTILO FOTOCHECK BLINDADO */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {usuarios && usuarios.length > 0 ? (
          usuarios.map(u => {
            ...
            return (
              ...
              >
                ...
                {/* CUERPO: MÉTRICAS OPERATIVAS SEGÚN ROL */}
          ...
          /* Mensaje de Fallback sutil por si la base de datos tarda en responder */
          <div className="col-span-full text-center py-12 text-slate-400 font-bold text-xs tracking-widest uppercase">
            Cargando credenciales del personal...
          </div>
        )}
      </div>
